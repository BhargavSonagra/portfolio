...
const About = () => {
  const skillRef = useRef(null); // use single ref for skill section
  const isInView = useInView(skillRef, { once: false, amount: 0.3 }); // repeat every time 30% in view
  ...
  return (
    <div className="mt-20 overflow-y-auto rounded-4xl">
      ...
      <div className="text-white px-6 sm:px-16 py-10 flex flex-col sm:flex-row gap-8 items-center justify-center w-full">
        {/* Left Section */}
        <motion.div
          className="w-full sm:w-1/2 text-center sm:text-left sm:text-xl"
        ...
        <div className="w-full sm:w-1/2">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-center md:items-center justify-center gap-6">
            {/* Skills List */}
            <motion.ul
              ref={skillRef}
              className="w-full sm:w-1/2 text-left text-lg sm:text-xl space-y-2"
